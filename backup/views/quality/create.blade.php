...
<head>
    ...
    <title>Registrar Inspección de Calidad - Metalúrgica Precision S.A.</title>
            ...
            <div class="container mx-auto px-6 py-4">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-gray-600">Registrar Inspección de Calidad</p>
                    </div>
        ...
        <main class="container mx-auto px-6 py-8">
            <div class="max-w-4xl mx-auto">
                <div class="bg-white rounded-lg shadow-md p-8">
                    <div class="mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Nueva Inspección de Calidad</h2>
                        <p class="text-gray-600 mt-1">Registre los resultados de la inspección de calidad</p>
                    </div>

                    <form action="{{ route('quality.store') }}" method="POST" id="qualityForm">
                        @csrf
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <optgroup label="Defectos Dimensionales">
                                        <option value="Fuera de Tolerancia" {{ old('defect_type') == 'Fuera de Tolerancia' ? 'selected' : '' }}>Fuera de Tolerancia</option>
                                        <option value="Medida Incorrecta" {{ old('defect_type') == 'Medida Incorrecta' ? 'selected' : '' }}>Medida Incorrecta</option>
                                        <option value="Deformación" {{ old('defect_type') == 'Deformación' ? 'selected' : '' }}>Deformación</option>
                                    </optgroup>
                                    <optgroup label="Defectos Superficiales">
                                        <option value="Rayado" {{ old('defect_type') == 'Rayado' ? 'selected' : '' }}>Rayado</option>
                                        <option value="Corrosión" {{ old('defect_type') == 'Corrosión' ? 'selected' : '' }}>Corrosión</option>
                                        <option value="Rebaba" {{ old('defect_type') == 'Rebaba' ? 'selected' : '' }}>Rebaba</option>
                                        <option value="Porosidad" {{ old('defect_type') == 'Porosidad' ? 'selected' : '' }}>Porosidad</option>
                                    </optgroup>
                                    <optgroup label="Defectos de Material">
                                        <option value="Grieta" {{ old('defect_type') == 'Grieta' ? 'selected' : '' }}>Grieta</option>
                                        <option value="Inclusión" {{ old('defect_type') == 'Inclusión' ? 'selected' : '' }}>Inclusión</option>
                                        <option value="Dureza Inadecuada" {{ old('defect_type') == 'Dureza Inadecuada' ? 'selected' : '' }}>Dureza Inadecuada</option>
                                    </optgroup>
                                    <optgroup label="Otros">
                                        <option value="Ensamblaje Defectuoso" {{ old('defect_type') == 'Ensamblaje Defectuoso' ? 'selected' : '' }}>Ensamblaje Defectuoso</option>
                                        <option value="Falta de Componente" {{ old('defect_type') == 'Falta de Componente' ? 'selected' : '' }}>Falta de Componente</option>
                                        <option value="Otros" {{ old('defect_type') == 'Otros' ? 'selected' : '' }}>Otros</option>
                                    </optgroup>
                                </select>
                                @error('defect_type')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-6 bg-purple-50 border border-purple-200 rounded-lg p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-purple-900" id="summary_total">--</p>
                                </div>
                                <div>
                                    <p class="text-xs text-purple-700">Porcentaje de Calidad</p>
                                    <p class="text-2xl font-bold text-purple-900" id="summary_quality">--%</p>
                                </div>
                                <div>
                                    <p class="text-xs text-purple-700">Tasa de Rechazo</p>
                                    <p class="text-2xl font-bold text-purple-900" id="summary_reject">--%</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="text-sm text-blue-700">
                                    ...
                                    <ul class="mt-1 list-disc list-inside">
                                        <li>El total inspeccionado se calcula automáticamente</li>
                                        <li>Aprobadas + Rechazadas = Total Inspeccionado</li>
                                        <li>Los porcentajes se calculan en base al total</li>
                                        <li>Las inspecciones afectan directamente el indicador de Calidad del OEE</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
