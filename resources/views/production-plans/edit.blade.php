...
<head>
    ...
    <title>Editar Plan #{{ $productionPlan->id }}</title>
        ...
        <header class="bg-white shadow-md">
            <div class="container mx-auto px-6 py-4">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">📋 Editar Plan #{{ $productionPlan->id }}</h1>
                        <p class="text-gray-600">Modifica los datos del plan de producción</p>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                @foreach($equipment as $eq)
                                    <option value="{{ $eq->id }}" {{ old('equipment_id', $productionPlan->equipment_id) == $eq->id ? 'selected' : '' }}>
                                        {{ $eq->name }} - {{ $eq->code }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    id="product_name" 
                                    name="product_name" 
                                    value="{{ old('product_name', $productionPlan->product_name) }}"
                                    class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    id="product_code" 
                                    name="product_code" 
                                    value="{{ old('product_code', $productionPlan->product_code) }}"
                                    class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input 
                                    ...
                                    value="{{ old('target_quantity', $productionPlan->target_quantity) }}"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition
                                    {{ old('shift', $productionPlan->shift) == 'morning' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                                    <input 
                                        type="radio" 
                                        name="shift" 
                                        value="morning"
                                        class="sr-only"
                                        {{ old('shift', $productionPlan->shift) == 'morning' ? 'checked' : '' }}
                                        required>
                                    <div class="flex-1">
                                ...
                                <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition
                                    {{ old('shift', $productionPlan->shift) == 'afternoon' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                                    <input 
                                        type="radio" 
                                        name="shift" 
                                        value="afternoon"
                                        class="sr-only"
                                        {{ old('shift', $productionPlan->shift) == 'afternoon' ? 'checked' : '' }}
                                        required>
                                    <div class="flex-1">
                                ...
                                <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition
                                    {{ old('shift', $productionPlan->shift) == 'night' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                                    <input 
                                        type="radio" 
                                        name="shift" 
                                        value="night"
                                        class="sr-only"
                                        {{ old('shift', $productionPlan->shift) == 'night' ? 'checked' : '' }}
                                        required>
                                    <div class="flex-1">
                                        <div class="text-2xl mb-1">🌙</div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    id="start_date" 
                                    name="start_date" 
                                    value="{{ old('start_date', $productionPlan->start_date->format('Y-m-d\TH:i')) }}"
                                    x-model="startDate"
                                    class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    required>
                                @error('start_date')
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    id="end_date" 
                                    name="end_date" 
                                    value="{{ old('end_date', $productionPlan->end_date->format('Y-m-d\TH:i')) }}"
                                    x-model="endDate"
                                    class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    required>
                                @error('end_date')
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                rows="4"
                                class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Información adicional sobre el plan...">{{ old('notes', $productionPlan->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('planForm', () => ({
                ...
                equipmentId: '{{ old('equipment_id', $productionPlan->equipment_id) }}',
                equipmentInfo: {
                    status: '',
                    capacity: 0
                },
                targetQuantity: {{ old('target_quantity', $productionPlan->target_quantity) }},
                startDate: '{{ old('start_date', $productionPlan->start_date->format('Y-m-d')) }}',
                endDate: '{{ old('end_date', $productionPlan->end_date->format('Y-m-d')) }}',
                loading: false,
