...
<head>
    ...
    <title>Registro de Producción - Metalúrgica Precision S.A.</title>
...
<body class="bg-gray-100">
    <div class="min-h-screen">
            ...
            <div class="container mx-auto px-6 py-4">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-gray-600">Registro de Producción</p>
                    </div>
        ...
        <main class="container mx-auto px-6 py-8">
            ...
            <!-- Filters -->
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                <form method="GET" action="{{ route('production.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Equipo</label>
                        <select name="equipment_id" class="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ request('start_date') }}" class="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Fecha Fin</label>
                        <input type="date" name="end_date" value="{{ request('end_date') }}" class="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        <a href="{{ route('production.index') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg transition">
                            Limpiar
                        </a>
                    </div>
                </form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div class="bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-800">{{ $productionData->total() }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-gray-600 text-sm">Total Producido</p>
                            <p class="text-2xl font-bold text-green-600">{{ number_format($productionData->sum('actual_production')) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-gray-600 text-sm">Unidades Buenas</p>
                            <p class="text-2xl font-bold text-purple-600">{{ number_format($productionData->sum('good_units')) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-red-600">{{ number_format($productionData->sum('defective_units')) }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">Defectuosas</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">Eficiencia</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($productionData as $data)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $data->production_date->format('d/m/Y H:i') }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-900">
                                    {{ number_format($data->actual_production) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-green-600">
                                    {{ number_format($data->good_units) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-red-600">
                                    {{ number_format($data->defective_units) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @php
                                        $efficiency = $data->planned_production > 0 ? ($data->actual_production / $data->planned_production) * 100 : 0;
                                        $colorClass = $efficiency >= 90 ? 'bg-green-100 text-green-800' : ($efficiency >= 75 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800');
                                    @endphp
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 text-lg">No hay registros de producción</p>
                                        <a href="{{ route('production.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition">
                ...
                <!-- Pagination -->
                @if($productionData->hasPages())
                    <div class="bg-gray-50 px-6 py-4">
                        {{ $productionData->links() }}
                    </div>
                @endif
            </div>
        </main>
